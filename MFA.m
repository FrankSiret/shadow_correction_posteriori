...
%% Dilataciones seguidas de erosiones
se = strel('disk',400); 
im = imclose(I,se);
waitbar(2/5,h);
...
lpf=imfilter(I,lf,'replicate','same');
Ca=-mean(mean(lpf));

%% Creando el componente aditivo y la eliminación del ruido aditivo
Sa=im + Ca;
Ne = I-Sa;
waitbar(3/5,h);
...
fig1=figure('Name','Filtrado Morfológico Aditivo','NumberTitle','off');
...
fig2=figure('Name','Imagen resultante del Filtrado Morfológico Aditivo','NumberTitle','off');
